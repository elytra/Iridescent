...
public class EntitySeat extends Entity {
	...
	protected void addPassenger(Entity passenger) {
		...
		if (passenger instanceof EntityPlayer) {
			...
			for (EntityPlayer player : world.playerEntities) {
				player.sendMessage(new TextComponentTranslation("iridescent.startLounging", component, pct).setStyle(new Style().setColor(TextFormatting.GOLD)));
			}
		}
	}
	...
	protected void removePassenger(Entity passenger) {
		...
		if (passenger instanceof EntityPlayer) {
			...
			int pct = (int)((ModuleChair.lounging.size()/(double)world.playerEntities.size())*100);
			for (EntityPlayer player : world.playerEntities) {
				player.sendMessage(new TextComponentTranslation("iridescent.stopLounging", component, pct).setStyle(new Style().setColor(TextFormatting.GOLD)));
			}
		}
	}
