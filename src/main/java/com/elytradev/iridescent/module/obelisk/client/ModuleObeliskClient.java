package com.elytradev.iridescent.module.obelisk.client;
...
import com.elytradev.iridescent.module.obelisk.TileEntityObelisk;
...
public class ModuleObeliskClient extends ModuleClient {
	...
	public void spark(TileEntityObelisk teo) {
		...
		double x = rtr == null ? teo.getPos().getX()+0.5 : rtr.hitVec.x;
		double y = rtr == null ? teo.getPos().getY()+0.5 : rtr.hitVec.y;
		double z = rtr == null ? teo.getPos().getZ()+0.5 : rtr.hitVec.z;
	...
	public void onStitch(TextureStitchEvent.Pre e) {
		e.getMap().registerSprite(new ResourceLocation("iridescent", "blocks/obelisk_floor_glowmap"));
		e.getMap().registerSprite(new ResourceLocation("iridescent", "blocks/totem_glowmap"));
		e.getMap().registerSprite(new ResourceLocation("iridescent", "blocks/totem_top_glowmap"));
	}
	
	@SubscribeEvent
	public void onTooltip(ItemTooltipEvent e) {
		if (e.getItemStack().getItem() == Items.BED) {
			e.getToolTip().add(I18n.format("iridescent.bedsAreDisabled"));
			e.getToolTip().add(I18n.format("iridescent.findObelisks"));
		}
	}
				...
				for (int z = -1; z <= 1; z++) {
					...
					for (TileEntity te : c.getTileEntityMap().values()) {
						if (te instanceof TileEntityObelisk) {
							...
							Vec3d b = new Vec3d(te.getPos().getX()+0.5, a.y, te.getPos().getZ()+0.5);
